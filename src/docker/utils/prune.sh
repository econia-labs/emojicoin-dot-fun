...
if [ -n "$reset_localnet" ]; then
	...
	docker compose -f compose.local.yaml --profile frontend --env-file example.local.env down --volumes
else
	docker compose -f compose.local.yaml --profile frontend --env-file example.local.env down
fi
