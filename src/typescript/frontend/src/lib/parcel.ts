...
export class Parcel<S> {
  ...
  async getData(to: number, count: number): Promise<S[]> {
    ...
    try {
      ...
      console.warn(
        "Could not get first event. This either means that no events have yet been emitted for this data type, or that the fetch first event function is wrong.",
        e
      );
      return [];
    }
