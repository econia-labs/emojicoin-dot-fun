...
const allServerAPIKeys: Record<Network, string | undefined> = {
...
const serverApiKey = allServerAPIKeys[APTOS_NETWORK];
if (typeof serverApiKey === "undefined" && typeof window === 'undefined') {
  // Do nothing if we're on a local network, because we don't need an API key for it.
  if (APTOS_NETWORK !== "local") {
    if (APTOS_NETWORK === "custom") {
      console.warn(`No API key set. Ignoring because we're on the \`${APTOS_NETWORK}\` network.`);
    } else {
      throw new Error(`Invalid API key set for the network: ${APTOS_NETWORK}: ${serverApiKey}`);
    }
  }
}
...
export const APTOS_SERVER_API_KEY = serverApiKey;
