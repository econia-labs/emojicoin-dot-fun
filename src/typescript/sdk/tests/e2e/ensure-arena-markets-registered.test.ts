...
import { ONE_SECOND_MICROSECONDS, setNextMeleeDurationAndEnsureCrank } from "./broker/utils";
...
describe("ensures the two arena markets and the arena module are on-chain as expected", () => {
  ...
  it("verifies an arena has already started with a duration of 15 seconds", async () => {
    expect(melee.view.duration).toEqual(ONE_SECOND_MICROSECONDS * 15n);
  });
  ...
  it("verifies enter+swap+exit procedure", async () => {
    ...
    const arenaEnters: ArenaEnterModel[] | null = (
      await postgrest
        .from(TableName.ArenaEnterEvents)
        .select("*")
        .eq("melee_id", melee.view.meleeID)
    ).data;
    let arenaPositions: ArenaPositionModel[] | null = (
      await postgrest.from(TableName.ArenaPosition).select("*").eq("melee_id", melee.view.meleeID)
    ).data;
    ...
    const arenaSwaps: ArenaSwapModel[] | null = (
      await postgrest.from(TableName.ArenaSwapEvents).select("*").eq("melee_id", melee.view.meleeID)
    ).data;
    arenaPositions = (
      await postgrest.from(TableName.ArenaPosition).select("*").eq("melee_id", melee.view.meleeID)
    ).data;
    ...
    const arenaExits: ArenaExitModel[] | null = (
      await postgrest.from(TableName.ArenaExitEvents).select("*").eq("melee_id", melee.view.meleeID)
    ).data;
    arenaPositions = (
      await postgrest.from(TableName.ArenaPosition).select("*").eq("melee_id", melee.view.meleeID)
    ).data;
    ...
    expect(position.deposits).toEqual(viewEnterEvent.inputAmount);
    expect(position.withdrawals).toEqual(
      viewExitEvent.emojicoin0Proceeds + viewExitEvent.emojicoin1Proceeds
    );
